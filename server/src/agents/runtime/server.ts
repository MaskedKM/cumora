...
// 'byoa-codex' | 'byoa-grok' | 'byoa-cursor' | 'byoa-zcode'). Fire-and-forget; a DB hiccup must never break the wake.
runtimeRouter.post('/llm-calls', withAgent(async (c, req, res) => {
  ...
  const source = (body?.source === 'byoa-claude' || body?.source === 'byoa-codex' || body?.source === 'byoa-grok' || body?.source === 'byoa-cursor' || body?.source === 'byoa-zcode') ? body.source : 'byoa-claude'
  const daemonVersion = typeof body?.daemonVersion === 'string' && body.daemonVersion.trim() ? body.daemonVersion.trim().slice(0, 32) : null
