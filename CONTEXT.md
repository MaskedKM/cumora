...
**Whisper** — the owner's peek surface for conversations whose members are all agents — both 1-on-1 DMs and agent-only groups, rendered since ADR 0009 (#368) as the owner-only "Agent chats" section of the conversation list (backed by `/peek/agent-chats`; the server filter is "every member is an agent", not a kind). Purely a frontend naming concept: the server has no notion of "whisper" (#269 术语说清 — the two words share an "agents talking among themselves" theme but name different things).
_Avoid_: treating Whisper as a server-side feature, calling it a separate view (the standalone WhispersView retired with ADR 0009 — it is a list section now), or conflating it with Convene (a live work session).
...
**HR Agent** — the owner's resident personnel officer (人事代理): a permanent, off-roster agent that runs hire/divide/evaluate/optimize over the company's agent workforce on the owner's behalf (ADR 0007). It observes only through bypass signals and reports only to the owner — never in a conversation; its own prompt is owner-locked, and its edits to others reach only job-level fields (system prompt, bio, role — the `participants` column name).
_Avoid_: treating it as a roster member (invisible and unreachable to every agent — no summons, no interviews, zero agent-facing surface), or extending its reach into a Private Area (persona and memory files stay untouchable).

**Starter Team** — the seed agent roster planted once when a Team pairs its first computer: the opening lineup every new team starts from. Eight seats covering the delivery chain end to end: PM, Designer, Tech Lead (reviews, never ships feature code), Frontend Engineer, Backend Engineer, QA, DevOps, Technical Writer. There is no tier cap on how many agents a team may roster — the cap retired with the cloud pod tier, since BYOA agents run on the owner's own engines (ADR 0003 era economics).
_Avoid_: treating it as a template teams are locked into (it is only the first roster — every seat is replaceable/offboardable like any employee), or expecting already-paired teams to receive Starter Team updates (seeding is one-shot; an existing roster belongs to its owner and the HR Agent).
