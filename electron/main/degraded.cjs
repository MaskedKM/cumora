...
//
//   stackd.children 非空 → 任一子进程 !running || circuitOpen = 降级
//   (单 unit 形态;stackd 本体活着才有 children)
//   否则旧三 unit 形态 → 有 unit 实际 active 才看 livez,非 ok = 降级
//   (8-31 事故形态:三件套停摆仅 sidecar 存活 → livez 死 → ⚠)
//
// 返回 null = 无法判定(报告缺字段/栈未装/unit 全停):调用方保持上一次
// 判定 —— 未知 ≠ 降级,与 StackTab「轮询出错保留最后已知值」同一取舍。
// 抽成无依赖纯函数正是为了这份规则能有真单测(wiring 型功能必须配断言)。
