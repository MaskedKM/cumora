...
- §team/§ — team project mountpoints, one symlink per project id (see the
...
2. Team projects you are a member of — real shared folders your team bound to
   Cumora. Each one appears in your home at §team/<project-id>/§ — that symlink
   ...
   §cumora project ls§ lists ids and names. If §team/§ is empty or missing on
   this machine, use the §cumora project§ CLI instead (read/write/append/edit/
   delete/mv/stat/grep).
...
- §cumora project ls§ — your team's shared project folders (real folders, same
  membership as the human UI). Prefer working directly under §team/<project-id>/§;
  the CLI (§cumora project read|write|append|edit|delete|mv|stat|grep§) works
