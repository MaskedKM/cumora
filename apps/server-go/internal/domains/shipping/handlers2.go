...
func verificationPatch(db *sql.DB) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		_ = recordEvent(r.Context(), db, companyID, feature.id, uid, "verification.updated",
...
func releaseCreate(db *sql.DB) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if _, err := db.ExecContext(r.Context(), `
			...
			id, feature.id, environment, nullStrPtr(body.optText("version", 200)), nullStrPtr(body.optText("commitSha", 200)),
...
func releaseAction(db *sql.DB) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if action == "approve" || action == "rollback" {
			if _, ok2 := httpx.ResolveCompanyRole(w, r, db, uid); !ok2 {
				return // 403 已写响应
			}
		}
...
func frictionPatch(db *sql.DB) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		_ = recordEvent(r.Context(), db, companyID, nullStrPtr2Str(featureID), uid, "friction.updated",
			map[string]any{"frictionId": r.PathValue("id")})
...
func regressionCreate(db *sql.DB) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if _, err := db.ExecContext(r.Context(), `
			...
			id, featureID, nullStrPtr(invariantID), nullStrPtr(sourceVerificationID),
