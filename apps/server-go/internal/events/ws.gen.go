...
var CompanyChannels = []string{
	ChMessageNew, ChMessageDelta, ChTyping, ChStatus, ChReactions, ChPolls, ChConvoUpdated, ChGroupPulled, ChConvene, ChBoards, ChDocs, ChDocMention, ChCalendarReminder, ChCalendarEvents, ChInbox, ChProject,
}
...
// ProjectFilesChangedEvent —— project.files_changed(通道 cumora:project、scope=company)。
// 团队工作区文件变更广播(#337,区级粒度,带文件清单不含内容)。发布方:server(挂载 watcher 上报处理 / 兜底扫描);帧路径对同租户成员可见,内容不随帧下发。
type ProjectFilesChangedEvent struct {
	Type      string           `json:"type"`
	CompanyID string           `json:"companyId"`
