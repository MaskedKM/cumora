...
func (f fakeDeps) deps() probe.Deps {
	return probe.Deps{
		...
		Dial: func(addr string) error {
			if f.closedPorts[strings.Split(addr, ":")[0]+":"+strings.Split(addr, ":")[1]] {
...
func TestDaemonTokenMissingIsWarn(t *testing.T) {
	r := Run((fakeDeps{envData: fullEnv}).deps(), testCfg()) // daemon.env fake 返回 fullEnv? 不,fake 只有一份 envData
	_ = r                                                    // daemon.env 走同一 ReadFile fake,fullEnv 无 ANTHROPIC_AUTH_TOKEN → warn
