...
4. **默认区转正为特殊项目**:每队一个 is_default 项目("团队文件"),
   置顶公共盘、全员可见、不可删除;agent 侧默认区消费链(附件落盘等)
   改锚到它。
...
6. **生命周期:无终点状态,仅删除**(ChatGPT 形态)。归档概念退役
   ...
   受管目录提示可手动清理)、关联与交付台账由 CASCADE 承担、is_default
   项目不可删。换盘(rebind)首版不做,记余量。
...
8. **实施三刀纵切**(一 PR 一票,串行):刀 1 模型并表+域重构
   (契约对外形状不变,纯内部换源)→ 刀 2 契约/CLI/daemon 改名
   (前端调用点同步,UI 形状不变)→ 刀 3 UI 升格+文档
   (CONTEXT.md 词条改写随此刀)。
...
- 既有裂缝(推导分叉/归档不级联/死入口/手填 id)随合并自然消亡,
  不再单独修。
- ADR 0006 的全部机制(挂载/防护/感知/交付面)语义不变,宿主改名。
