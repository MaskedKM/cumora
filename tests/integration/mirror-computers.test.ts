...
// #337 挂载文件感知上报面:device token 调 /api/computers/me/project-report,
// server 对账已知态(Redis cumora:wsidx:<wsId>)→ 变化项快照(.cumora/versions/)
// → 广播 project.files_changed(cumora:project 通道帧 = wsx 桥的输入,
...
test('[mirror] project-report: dedup, snapshot, files_changed frame', async () => {
  const code = (await call('/computers', { method: 'POST', body: '{}' })).json.code
  const paired = (await call('/computers/pair', {
  ...
  await sub.subscribe('cumora:project')

  const post = (items: unknown) =>
    fetch(`${mirrorBase()}/api/computers/me/project-report`, {
      method: 'POST',
  ...
  let res = await post([{ projectId: wsId, path: 'note.md' }])
  ...
  res = await post([{ projectId: wsId, path: 'note.md' }])
  assert.equal((await res.json() as { changed: number }).changed, 1)
  res = await post([{ projectId: wsId, path: 'note.md' }])
  ...
  // 广播帧:project.files_changed,清单含 note.md。
  await new Promise((r) => setTimeout(r, 300))
  const evt = frames.find((f) => f.type === 'project.files_changed' && f.projectId === wsId)
  ...
  assert.ok(frames.some((f: any) => f.type === 'project.files_changed' &&
    f.projectId === wsId && f.changes.some((c: any) => c.path === 'note.md' && c.removed === true)),
  ...
  res = await post([{ projectId: otherWs, path: 'secret.md' }])
  assert.equal(res.status, 200)
  assert.equal((await res.json() as { changed: number }).changed, 0, 'cross-tenant report silently ignored')

  // 401:无 device token。
  const noAuth = await fetch(`${mirrorBase()}/api/computers/me/project-report`, {
