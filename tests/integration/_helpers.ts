 ...
 * (CUMORA_MIRROR_BASE,由 tests/integration/run.mjs 自建自起),
...
import { env } from './harness/env.js'
  ...
  if (!schemaReady) {
    schemaReady = (async () => {
      ...
      throw new Error(
        ...
        '(tests/integration/run.mjs does this for you; it applies 0001_baseline.sql on boot)',
      )
    })()
  }
  return schemaReady
...
export async function teardownAll(server?: import('node:http').Server): Promise<void> {
  ...
  try {
    const { redis, sub } = await import('./harness/redis.js')
    redis.disconnect()
    sub.disconnect()
  } catch { /* ignore */ }
}
...
export function startMirror(user: string, company: string): {
  ...
  if (!MIRROR_BASE) {
    throw new Error(
      ...
      'Run via `npm run test:integration` (tests/integration/run.mjs boots the Go server for you).',
    )
  }
  return {
    baseUrl: () => MIRROR_BASE,
